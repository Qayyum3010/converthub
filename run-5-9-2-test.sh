...
test_conversion() {
  ...
  STATUS="processing"
  ATTEMPTS=0
  while [ "$STATUS" != "done" ] && [ "$STATUS" != "failed" ] && [ $ATTEMPTS -lt 10 ]; do
    sleep 1
...
FORMATS=(jpg png webp gif tiff avif)

for src in "${FORMATS[@]}"; do
  for tgt in "${FORMATS[@]}"; do
    if [ "$src" != "$tgt" ]; then
      test_conversion "test-fixtures/sample-image.${src}" "${src}" "${tgt}"
    fi
  done
done

echo "=== All 5.9.2 tests complete ==="
